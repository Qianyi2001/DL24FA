...
class JEPAModel(nn.Module):
    ...
    def training_step(self, states, actions):
        ...
        contrastive_losses = []
        for t in range(T):
            contrastive_losses.append(self.contrastive_loss(pred_repr[:, t, :], target_repr[:, t, :]))

        contrastive_loss = torch.mean(torch.stack(contrastive_losses))
        ...
        total_loss = contrastive_loss + 0.01 * variance_loss + 0.005 * cov_loss
